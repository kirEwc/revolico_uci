...
export default function RegistroPage() {
  ...
  const [step, setStep] = useState<"datos" | "code">("datos");
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/send-code", {
        ...
        body: JSON.stringify({ email: formData.email, isRegistro: true }),
      });
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-muted-foreground">
            {step === "datos" ? "Crea tu cuenta de vendedor" : "Ingresa el código de verificación"}
          </p>
        </div>
          ...
          {step === "datos" ? (
            ...
            <form onSubmit={handleVerifyCode}>
              ...
              <div className="mb-4 p-3 bg-secondary/50 rounded-lg space-y-1">
                <p className="text-sm text-muted-foreground">Enviamos un código a:</p>
                <p className="font-medium">{formData.email}</p>
                <p className="text-xs text-muted-foreground">Nombre: {formData.nombre}</p>
              </div>
